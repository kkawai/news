...
/**
 * 
 * A great alternative to the apache-commons and android-extended
 * HttpDefaultClient
 * 
...
public final class HttpMessage {
	...
	private static final String TAG = HttpMessage.class.getName();
	...
	public String getString() throws Exception {
		...
		final int status = ((HttpURLConnection) urlConnection).getResponseCode();
		if (status >= 400)
	...
	public String getString(final String optionalHeader, final String optionalHeaderValue) throws Exception {
		final URLConnection urlConnection = openConnection();
		assignCommonAttributesToHttpURLConnection(urlConnection, true, false);
		((HttpURLConnection) urlConnection).setRequestMethod("GET");
		urlConnection.setRequestProperty("Content-Type", "text/plain;charset=utf-8");
		urlConnection.setRequestProperty("Accept-Encoding", "gzip");
		urlConnection.setRequestProperty(optionalHeader, optionalHeaderValue);
	 ...
	 * @throws Exception
	 */
	public String getString(final int maxLength, final int timeoutSeconds) throws Exception {
		final URLConnection urlConnection = openConnection();
		assignCommonAttributesToHttpURLConnection(urlConnection, true, false, timeoutSeconds);
		((HttpURLConnection) urlConnection).setRequestMethod("GET");
		urlConnection.setRequestProperty("Content-Type", "text/plain;charset=utf-8");
		urlConnection.setRequestProperty("Accept-Encoding", "gzip");
		final InputStream is = urlConnection.getInputStream();

		final String string = getString(is, maxLength);
		is.close();
		return string;
	}
	...
	private void assignCommonAttributesToHttpURLConnection(final URLConnection urlConnection, final boolean doInput, final boolean doOutput) {
		assignCommonAttributesToHttpURLConnection(urlConnection, doInput, doOutput, 15);
	}
	
	private void assignCommonAttributesToHttpURLConnection(final URLConnection urlConnection, final boolean doInput, final boolean doOutput, final int timeoutSeconds) {
	...
	public String post(final List<NameValuePair> formParams) throws Exception {

		final StringBuilder params = new StringBuilder();
		if (formParams != null) {

			for (final NameValuePair pair : formParams) {
				params.append(URLEncoder.encode(pair.getName(), UTF8)).append('=').append(URLEncoder.encode(pair.getValue(), UTF8)).append('&');
			}
			params.deleteCharAt(params.length() - 1);
		}
		return postData(params.toString().getBytes(UTF8), "POST");
	...
	private String getString(final InputStream is, final int maxStringSize) throws Exception {

		final StringBuilder sb = new StringBuilder(maxStringSize);
		final byte buf[] = new byte[maxStringSize];
		int read = 0;
		while ((read = is.read(buf)) != -1) {
			sb.append(new String(buf, 0, read, UTF8));
		}
