...
import com.initech.news.view.drag.transformer.Transformer;
import com.initech.news.view.drag.transformer.TransformerFactory;
...
public class DraggableView extends RelativeLayout {
   ...
   public void closeToRight() {
      if (viewDragHelper.smoothSlideViewTo(dragView, getWidth(), getHeight() - transformer.getMinHeightPlusMargin())) {
         ViewCompat.postInvalidateOnAnimation(this);
   ...
   public boolean onTouchEvent(MotionEvent ev) {
      ...
      if (!lockDragMode) {
         try {
            ...
            MLog.e(TAG, "viewDragHelper.processTouchEvent() failed", e);
            return true;
         }
      }
      if (isClosed()) {
   ...
   protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
      Log.e("drview", "onLayout " + changed + " | " + left + " | " + top + " | " + right + " | " + bottom);
      Log.e("drview", "view size " + dragView.getWidth() + " | " + dragView.getHeight() + " + screen width " + getWidth());
      if (isInEditMode()) {
   ...
   void changeSecondViewPosition() {
      /*
      * MLog.i(TAG, "changeSecondViewPosition() orig height: " + transformer.getOriginalHeight() + " dragView height: " + dragView.getMeasuredHeight
      * () + " dragView.bottom: " + dragView.getBottom());
      *
      * Add youtube effect when dragging mini-player down in portrait.
      */
      ViewHelper.setY(secondView, dragView.getBottom() + ((dragView.getBottom() - dragView.getMeasuredHeight()) / 3.1f));
   }
   ...
   void changeSecondViewAlpha() {
      ...
      setBackshadowAlpha(1 - getVerticalDragOffset());
   }
   ...
   void changeDragViewViewAlpha() {
      if (enableHorizontalAlphaEffect) {
         ...
         DragAudioManager.getInstance().adjustVolume(alpha);
      }
   }
   ...
   private void initializeTransformer(TypedArray attributes) {
      ...
      transformer.setViewHeight(attributes.getDimensionPixelSize(R.styleable.draggable_view_top_view_height, DEFAULT_TOP_VIEW_HEIGHT));
      ...
      transformer.setMarginRight(attributes.getDimensionPixelSize(R.styleable.draggable_view_top_view_margin_right, DEFAULT_TOP_VIEW_MARGIN));
   ...
   private boolean smoothSlideTo(float slideOffset, final boolean isAnimate) {
      ...
      Log.e("drview", "minimise " + x + " + " + transformer.getMinWidthPlusMarginRight() + " = " + getWidth());
   ...
   public void setIsDragging(final boolean isDragging) {
      ...
      MLog.i(TAG, "setIsDragging() " + isDragging);
      if (isDragging) {
         listener.onDragging();
      } else {
         listener.onDraggingStopped();
      }
   }

   public void setBackshadowView(final View backshadowView) {
      this.backshadowView = backshadowView;
   }

   private void setBackshadowAlpha(final float alpha) {
      if (backshadowView == null)
         return;
      backshadowView.setAlpha(alpha);
   }
}
